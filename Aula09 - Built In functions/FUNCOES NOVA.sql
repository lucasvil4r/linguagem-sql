...
SELECT NOME,  LTRIM(NOME) [LTRIM] FROM PESSOA  where ltrim(nome) =  'ARNALDO FELIPE'
...
SELECT CPF, LEN(CPF) [LEN] FROM PESSOA where len(cpf) < 10
...
SELECT 
cpf, concat(replicate(0,11 - len(cpf)),cpf)
FROM PESSOA
...
DATEPART(dd,DATA_NASCIMENTO)DIA ,  --BUSCA DIA,
DATEPART(DAY,DATA_NASCIMENTO)DIA ,  --BUSCA DIA,
DATEPART(YEAR,DATA_NASCIMENTO) ANO  --BUSCA ANO
...
,DATEDIFF(day,DATA_NASCIMENTO,'2001-09-11') DIA_11_SETEMBRO  
FROM PESSOA
...
DATEADD(MONTH,-1,EOMONTH(GETDATE())) ULTIMO_DIA_MES_PASSADO
FROM PESSOA
...
SELECT CONVERT(DECIMAL(10,2),REPLACE('2,5',',','.'))
...
' O ID PESSOA : ' + CAST (ID_PESSOA AS VARCHAR(1)) 
+ ' NASCEU EM ' + CAST (DATA_NASCIMENTO AS VARCHAR(100)) CONCATENA_ID_NASCEU
FROM PESSOA
...
SELECT FLOOR(SALARIO)[FLOOR], ceiling(salario), salario FROM PESSOA
...
THEN 'EMAIL VALIDADO' 
ELSE 'EMAIL NÃO VALIDADO' END VALIDACAO
FROM PESSOA WHERE CASE WHEN CHARINDEX('@',EMAIL) > 0 
THEN 'EMAIL VALIDADO' 
ELSE 'EMAIL NÃO VALIDADO' END = 'EMAIL VALIDADO'
...
SELECT *,RANK() OVER (ORDER BY SALARIO) RANKING ,RANK() OVER (ORDER BY DATA_NASCIMENTO) RANKING_IDADE ,DATA_NASCIMENTO,
RANK() OVER (ORDER BY SALARIO DESC) RANKING_MAIOR_P_MENOR
FROM PESSOA 
...
INTO #CPF_DUPLICADO
 ...
 SELECT SALARIO, ROW_NUMBER() OVER (ORDER BY SALARIO DESC,DATA_NASCIMENTO) MAIOR_SALARIO
FROM PESSOA 
...
ROW_NUMBER() OVER (PARTITION BY CPF ORDER BY ID_PESSOA DESC) ROW2
 ...
 SELECT CPF,ROW_NUMBER() OVER (PARTITION BY CPF ORDER BY ID_PESSOA DESC) RANKING,ID_PESSOA
 INTO #DEDUP_TABLE
 FROM PESSOA

 DELETE FROM PESSOA WHERE ID_PESSOA IN (
 SELECT ID_PESSOA FROM #DEDUP_TABLE WHERE RANKING > 1
 )
